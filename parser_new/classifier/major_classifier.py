...
class train_major_classifier(object):
	...
	def load_major(self):
		...
		temp_major_list = db.major.find()
		self.major_list = self.major_list + [x['major'] for x in temp_major_list]

	def load_negative(self):
		path = "../docs/data/training/labelled/"#you should change this for your git location
		file = os.listdir(path)
		keyword=set(["basic_info","work_experience","project_experience","social_experience"])
	...
	def split_degree(self, row):
		degree_rex = u'学士|硕士|本科|博士|研究生|专业'
		return re.split(degree_rex, row)[0]

	def preprocess(self):
		self.load_major()
		self.load_negative()
		self.label = np.ones(len(self.negative_list)+len(self.major_list),dtype='int')
		self.label[:len(self.negative_list)] = 0
		self.corpus=[" ".join(jieba.lcut(i)) for i in self.negative_list + self.major_list]
	...
	def predict(self, new_word):
		...
		new_word = self.split_degree(new_word)
		if new_word in self.major_list:
			return 1 
...
if __name__ == "__main__":
	temp = train_major_classifier()
	temp.train()
	test_path = "../docs/data/test/labelled/"
	output_path = "/Users/kshen/Desktop/major/"
